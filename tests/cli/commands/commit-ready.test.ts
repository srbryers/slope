...
describe('collectCommitReady (GH #314)', () => {
  ...
  it('does not use historical CODEBASE commits for ignored current maps (#510)', async () => {
    const previousThreshold = process.env.SLOPE_CODEBASE_STALE_AT;
    process.env.SLOPE_CODEBASE_STALE_AT = '1';
    try {
      ...
      if (previousThreshold === undefined) delete process.env.SLOPE_CODEBASE_STALE_AT;
      else process.env.SLOPE_CODEBASE_STALE_AT = previousThreshold;
    }
  });
