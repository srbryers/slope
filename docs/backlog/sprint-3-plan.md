...
**Theme:** Publish @srbryers/mcp-tools, wire SLOPE into Cursor by default, and polish docs.

---

## Pre-Round Briefing

Sprint 2 delivered provider-agnostic init (--cursor, --generic), auto-card, and next-sprint detection. The new `@srbryers/mcp-tools` package is in the repo and builds; it has not been published to npm yet. This sprint gets the MCP server published, makes `slope init --cursor` optionally add SLOPE as an MCP server in Cursor, adds tests for mcp-tools, and brings README/docs in line with the three-package layout.
...
### S3-1: Publish @srbryers/mcp-tools and @srbryers/cli to npm
...
- **Description:**
  ...
  - Ensure [packages/cli/package.json](packages/cli/package.json) is at 0.4.0 and depends on `@srbryers/core@^0.3.3`.
  - Add a short **Publishing** section to the root [README](README.md) or to [docs/backlog/README.md](docs/backlog/README.md): list order (core first if needed), then `pnpm --filter @srbryers/mcp-tools build && npm publish -w packages/mcp-tools --access public`, then CLI; note that 2FA/OTP is required (`npm publish --otp=<code>`).
- **Acceptance:** Version and metadata are correct; maintainer can run publish steps from the doc without guessing.

### S3-2: slope init --cursor adds optional MCP config for @srbryers/mcp-tools
...
- **Description:**
  - When `slope init --cursor` runs, optionally add or merge into `.cursor/mcp.json` an entry for the `slope` MCP server that runs `npx @srbryers/mcp-tools` (or `mcp-slope-tools` from bin).
...
### S3-3: Unit tests for @srbryers/mcp-tools
...
- **Acceptance:** `pnpm --filter @srbryers/mcp-tools test` (or equivalent) passes; new contributors can run tests before publishing.
...
- **Description:**
  - In README, add `@srbryers/mcp-tools` to the Packages table with one-line description.
