...
| S146.1-5 | Wedge | Green | — | Validated release readiness locally with build, typecheck, full test suite, version smoke, roadmap/docs/map checks, and review findings clean. PR checks then exposed a CodeQL alert in the new staging helper before merge. |
| S146.1-6 | Wedge | Green | Rough: GitHub Advanced Security flagged the new version-bump staging helper because it built a shell command and escaped quotes without escaping backslashes. | Raised #528, replaced shell-quoted git add staging with execFileSync argv-form execution, and reran focused version tests, typecheck, and build before pushing the CI fix. |

### Conditions

| Condition | Impact | Description |
|---|---|---|
| rough | moderate | The release readiness gates found two SLOPE-local issues (#526 before PR creation and #528 from CodeQL after PR creation); both were fixed and folded into the release sprint. |
...
| Rough | S146.1-6 | GitHub Advanced Security flagged the new version-bump staging helper because it built a shell command and escaped quotes without escaping backslashes. |
...
- Release automation should avoid custom shell quoting for staged paths; argv-form git execution is simpler and safer.
...
| Lessons | Release automation should pass git path arguments by argv instead of custom shell quoting. | Version-bump staging now uses execFileSync with `['add', ...paths]`, eliminating the CodeQL escaping alert. |
...
| testing | healthy | After fixing #528, `corepack pnpm vitest run tests/cli/version.test.ts`, `corepack pnpm typecheck`, and `corepack pnpm build` passed. |
...
- Created GitHub issue #528 for the CodeQL alert on shell-quoted version-bump staging.
- Initial release commits included 415bc4e, 6def39b, 1128c3e, and 224df13; the CodeQL #528 fix was added after PR release validation.
- Review recommendation: architect required, code optional; manual architect/code review found no findings before PR, and CodeQL caught #528 during PR release validation.
- The release PR should include `Closes #524`, `Closes #525`, `Closes #526`, and `Closes #528`.
- After merge, create GitHub Release `v1.58.1`, watch the publish workflow, and verify npm reports @slope-dev/slope 1.58.1.

### 19th Hole

- **How did it feel?** A small release round with useful pressure: the release gates did exactly what they should and flushed out two local SLOPE issues before the package left the clubhouse.
