...
export class ClaudeCodeAdapter implements HarnessAdapter {
  ...
  installGuards(cwd: string, guards: AnyGuardDefinition[]): void {
    ...
    if (!existsSync(dispatcherPath)) {
      // Resolve the slope binary: prefer local node_modules/.bin, fall back to npx
      const localBin = join(cwd, 'node_modules', '.bin', 'slope');
      const slopeCmd = existsSync(localBin)
        ? '"$CLAUDE_PROJECT_DIR"/node_modules/.bin/slope'
        : 'npx --yes slope';
      const script = [
        '#!/usr/bin/env bash',
