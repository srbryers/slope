...
export const NARRATOR_SEGMENTS: Record<string, { text: string; label: string }> = {
  ...
  '3d': { label: 'bottleneck', text: "You can see it's digging into the specifics here, trying to understand what really matters for this project. It's going to remember things like this when it puts the roadmap together." },
  '3e': { label: 'gap',      text: "And it's also picking up on gaps it found during the profile scan, things that might be missing from the project. It'll ask about those directly so it knows what to prioritize." },
  ...
  '4c': { label: 'pushback', text: "So here you can see the user is pushing back on a few things. Small corrections, but these are going to drive every sprint after this, so it's really worth getting right." },
  ...
  '5c': { label: 'sprints',  text: "And then you've got the rest of the sprints, each one focused on a different area of the project. They each get their own sprint with real tickets, ready to go." },
