...
- \`/wrap-session\` — session close: standup wrap, unpushed-work check, session end, handoff
...
function resolveSlopeBin() {
  const localBin = join(process.cwd(), 'node_modules', '.bin', 'slope');
  if (existsSync(localBin)) return localBin;
  const devDist = join(process.cwd(), 'dist', 'cli', 'index.js');
  const packageJson = join(process.cwd(), 'package.json');
  if (existsSync(devDist) && existsSync(packageJson)) {
    try {
      const pkg = JSON.parse(readFileSync(packageJson, 'utf8'));
      if (pkg.name === '@slope-dev/slope') return \`node \${devDist}\`;
    } catch {}
  }
  try {
    execSync('command -v slope', { stdio: 'ignore' });
    return 'slope';
  } catch {}
  return 'npx --yes @slope-dev/slope';
}

export default async ({ $ }) => {
  const slope = resolveSlopeBin();
  return {
    event: async ({ event }) => {
      switch (event.type) {
