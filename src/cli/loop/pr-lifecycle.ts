...
export function hasCommitsAhead(branch: string, cwd: string): boolean {
  try {
    const count = execSync(`git rev-list --count main..${branch}`, {
      cwd,
      encoding: 'utf8',
...
- Generated by autonomous loop (\`slope loop run\`)`;
  ...
  try {
    const url = execSync(
      `gh pr create --base main --head "${branch}" --title "feat(${sprintId}): ${title}" --body "${body.replace(/"/g, '\\"')}"`,
...
export function runStructuralReview(
  ...
  let diff: string;
  try {
    diff = execSync(`gh pr diff ${prNumber}`, { cwd, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
  } catch {
  ...
  const consoleLogLines = diff.split('\n').filter(l => {
    if (!l.startsWith('+') || l.startsWith('+++')) return false;
    return /console\.log/.test(l);
  });
  // Filter: only count if not in test files (approximate by checking surrounding diff context)
  const prodConsoleLogs = consoleLogLines.length; // Simplified — structural review approximation
  if (prodConsoleLogs > 0) {
    addFinding('code', sprintId, sprintNum, 'minor', `${prodConsoleLogs} console.log statement(s) in diff`, cwd, log);
    findingCount++;
  }
  ...
  try {
    const filesJson = execSync(`gh pr view ${prNumber} --json files --jq '[.files[] | select(.additions > 500)] | .[].path'`, {
      cwd, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'],
    }).trim();
    if (filesJson) {
...
function addFinding(
  ...
  try {
    execSync(
      `pnpm slope review findings add --type=${type} --ticket=${sprintId}-0 --severity=${severity} --description="${description.replace(/"/g, '\\"')}" --sprint=${sprintNum}`,
      { cwd, stdio: 'pipe' },
    );
  } catch {
...
export function autoMerge(
  ...
  if (findingCount > 0) {
    try {
      const findingsList = execSync('pnpm slope review findings list', { cwd, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
    ...
    } catch { /* ok — proceed if findings list fails */ }
  }
  ...
  try {
    const filesChanged = execSync(`gh pr view ${prNumber} --json changedFiles --jq '.changedFiles'`, {
      cwd, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'],
    }).trim();
  ...
  try {
    execSync(`gh pr merge ${prNumber} --squash --delete-branch`, { cwd, stdio: 'pipe' });
...
function block(prNumber: number, reason: string, cwd: string, log: Logger): MergeResult {
  ...
  try {
    execSync(`gh pr comment ${prNumber} --body "Auto-merge blocked: ${reason}. Manual review required."`, {
      cwd, stdio: 'pipe',
    });
  } catch { /* ok */ }
